...
class Pyttsx3TTSService(BaseTTSService):
    ...
    async def synthesize(self, text: str, voice_id: Optional[str] = None, rate: int = 180) -> Path:
        ...
        # Generate cache key
        cache_key = hashlib.sha1(f"{voice_id or ''}|{rate}|{text}".encode("utf-8")).hexdigest()[:16]
    ...
    def _synthesize_sync(self, text: str, voice_id: Optional[str], rate: int, output_path: Path) -> None:
