...
from ..audio.constants import SAMPLE_RATE, CHANNELS, FRAME_SAMPLES, TTS_QUEUE_SIZE
...
class MediaStreamer:
    ...
    async def start_streaming(self, websocket: WebSocket, call_state: CallState) -> None:
        ...
        if self.stt_service.is_available:
            ...
            self._tts_task = asyncio.create_task(self._tts_consumer(websocket, call_state))
    ...
    async def play_audio_file(
        self, file_path: str, websocket: WebSocket, call_state: CallState
    ) -> None:
    ...
    async def _start_microphone_streaming(
        self, websocket: WebSocket, call_state: CallState
    ) -> None:
        ...
        self._tx_task = asyncio.create_task(self._tx_sender(websocket, call_state))
    ...
    async def _tx_sender(self, websocket: WebSocket, call_state: CallState) -> None:
        ...
        try:
            while True:
                ...
                payload = {
                    "kind": "audioData",
                    "audioData": {"data": b64_data, "sequenceNumber": sequence_number},
                }
            ...
            while True:
                ...
                try:
                    ...
                    wav_path = await self.tts_service.synthesize(
                        text=transcript, voice_id=None, rate=180
                    )
