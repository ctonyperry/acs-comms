...
def ensure_16k_mono_wav(path: str | Path) -> str:
    ...
    if not shutil.which("ffmpeg"):
        raise RuntimeError(
            f"{path_str} is not 16k mono s16le and ffmpeg is not installed to convert it."
        )
...
async def stream_wav_over_ws(
    ws: WebSocket, path: str, seq_start: int = 0, frame_ms: int = FRAME_MS
) -> int:
    ...
    with wave.open(path, "rb") as w:
        assert (
            w.getnchannels() == 1 and w.getframerate() == SAMPLE_RATE and w.getsampwidth() == 2
...
def crossfade_wav_files(
    wav_files: list[str | Path], crossfade_ms: int = 50, output_path: str | Path | None = None
) -> str:
        ...
        try:
            with wave.open(str(wav_file), "rb") as w:
                if w.getnchannels() != 1 or w.getframerate() != 16000 or w.getsampwidth() != 2:
                    raise ValueError(
                        f"All WAV files must be 16kHz mono s16le format. "
                        f"File {wav_file} is {w.getframerate()}Hz, "
                        f"{w.getnchannels()} channels, {w.getsampwidth()*8}-bit"
                    )
        except Exception as e:
            raise ValueError(f"Error reading {wav_file}: {e}") from e
    ...
    if len(wav_files) == 2:
        ...
        for i in range(len(wav_files) - 1):
            if i == 0:
                ...
                # Middle crossfades
                filter_complex += (
                    f"[cf{i-1}][{i+1}]acrossfade=d={crossfade_sec}:c1=tri:c2=tri[cf{i}];"
                )
    ...
    cmd = (
        ...
        + input_args
        + ["-filter_complex", filter_complex, "-map", "[out]", "-c:a", "pcm_s16le", output_path]
    )
